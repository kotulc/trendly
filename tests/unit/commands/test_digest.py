"""Unit tests for digest file writing and store bookkeeping."""

import frontmatter

from trendly.commands import digest
from trendly.config import data_dir
from trendly.models.article import Article
from trendly.services import store


def test_digest_writes_frontmatter_file(sample_topic):
    """Digest files carry url/tags/score frontmatter over the markdown body."""
    article = Article(url="http://x/a", title="Big News", markdown="Body text.",
                      tags=["ai"], score=0.7, summary="Sum.")
    out = digest.DigestCommand()(digest.DigestInput(articles=[article]),
                                 digest.DigestParams(topic=sample_topic))

    post = frontmatter.load(out.paths[0])
    assert post["url"] == "http://x/a" and post["tags"] == ["ai"] and post["score"] == 0.7
    assert post.content == "Body text."
    assert "big-news" in out.paths[0]


def test_digest_records_article_and_source(sample_topic):
    """Each digest write upserts the article row and bumps its source domain."""
    article = Article(url="http://news.test/a", title="T", markdown="Body.")
    digest.DigestCommand()(digest.DigestInput(articles=[article]),
                           digest.DigestParams(topic=sample_topic))

    con = store.connect(data_dir() / "trendly.db")
    assert con.execute("SELECT status FROM articles").fetchone() == ("digested",)
    assert con.execute("SELECT domain, kept FROM sources").fetchone() == ("news.test", 1)
